 ...
 * {@link com.opencqrs.esdb.client.EsdbClient#query(EventQuery, EventQueryRowHandler, EventQueryErrorHandler)}.
 */
public interface EventQueryErrorHandler {
     ...
     */
    void queryProcessingError(EventQueryProcessingError error);

    /**
     * Signals a marshalling error for result rows successfully received from the underlying event store that could not
     * be properly deserialized or transformed to the desired target type by means of the requested
     * {@link EventQueryRowHandler}.
     *
